...
def create_chroma_from_documents(_texts, _embeddings, _collection_name):
    return Chroma.from_documents(_texts, _embeddings, _collection_name)
...
def load_and_process_documents(path):
    ...
    texts = split_documents(
        CharacterTextSplitter(chunk_size=200, chunk_overlap=0), documents
    )
    chroma = create_chroma_from_documents(texts, embeddings, "us_citizen")
    return RetrievalQA.from_chain_type(
        llm=llm, chain_type="stuff", retriever=chroma.as_retriever()
    )
